...
def compute_volume_mm3(roi_mask, spacing):
    """
    Compute volume in mm³ instead of cc.

    Parameters:
    - roi_mask: 3D boolean/label mask of the ROI
...
def extract_group2_metrics(Dose_data, ROIs_data, target_labels=None, prescribed_dose=None):
    ...
    for roi_number, roi_info in ROIs_data["ROIs"].items():
        ...
        volume_mm3 = compute_volume_mm3(roi_mask, spacing)
...
def save_dose_histograms(group2_metrics, output_dir, bins=30):
    ...
    for roi_name, metrics in group2_metrics.items():
        ...
        # Define bin edges manually for wider spread
        min_dose = min(expanded)
        max_dose = max(expanded)
        bin_edges = np.arange(0, 41, 1)  # Bins at 0, 1, 2, ..., 40



        ax.hist(expanded, bins=bin_edges, color='skyblue', edgecolor='black')
