...
def show_dose_map(rd_data):  # this function opens and displays the dose maps in the form of a heat map.
...
def get_contour_data1(rs_dataset):  # Extracting the contour positioning acording to ROI and the color of each contour
    contour_data = {}
    contour_color = {}
    ...
    for i in range(len(structure_set_ROI_sequence)):
        ...
        for contour_sequence_item in contour_sequence:
            ...
            contour_data[roi_name] = [contour_points]  # why?
...
def load_contour_slices(
        rs_dataset):  # extracting from the RS file the data of each slice and organizing it according to ROIs
    contour_slices = {}
    roi_contour_sequence = rs_dataset.ROIContourSequence
    structure_set_ROI_sequence = rs_dataset.StructureSetROISequence
...
def load_ct_slices(ct_folder_path):  # loding all ct images from the folder and extracting the step sizes
...
def sort_ct(ct_slices):  # sorting the CT images in an increasing order, the most bottom image is first...
        ...
        for roi_name, matched_slice_list in matched_slices.items():
            for slice_number, contour_points, ct_image, slice_position in matched_slice_list:
                if slice_position[2] == slices_order[n]:
                    ...
                    plt.imshow(resampled_dose1 * grid_scaling, cmap='cool', origin='lower', vmin=0, vmax=Vmax1,
                               alpha=0.5)
...
(CT_slices, X_spacing, Y_spacing) = load_ct_slices('64147_SRS_3 METS_24082022')  # load CT slices
