...
# ROIs we want to completely ignore
SKIP_ROI_BASE = {"HO", "HOP"}

def should_skip_roi(name: str) -> bool:
    """
    Return True if this ROI name should be ignored (HO / HOP variants).
    """
    if not name:
        return False
    n = name.strip().upper()
    ...
    # Exact match: "HO", "HOP"
    if n in SKIP_ROI_BASE:
        return True

    # Prefix variants: "HOP LT", "HOP RT", "HO LEFT", etc.
    for base in SKIP_ROI_BASE:
...
def extract_ROIs_data(RS_data, CT_data):
    ...
    # ---------- FIRST PASS: define ROIs (skip HO / HOP) ----------
    for ROI in ROIs_raw_data:
        name = ROI.ROIName
        if should_skip_roi(name):
            # Completely ignore these ROIs
            # (no entry, no volume, no colors, no DVH)
            continue
    ...
    for contour in contours_data:
        ...
        # If the ROI was skipped above, it won't be in the dict
        if ROI_Number not in ROIs_data['ROIs']:
            continue
...
def extract_group1_metadata(RP_data, RD_data, patient_folder_path) -> Dict[str, Any]:
    ...
    *The function is resilient to missing DICOM tags and non‑standard planners –
    ...
    if rx is not None:
        meta["Prescription Dose or Max Dose[Gy]"] = float(rx)
...
def compute_roi_metrics(masks: Dict[str, np.ndarray],
                        ...
                        smooth_sigma: float,
                        log,
                        whole_brain: bool = False) -> pd.DataFrame:

    """
    Compute per-ROI dose/volume metrics and selected Vx (cc) for healthy brain.
    Before reporting healthy brain Vx we subtract the volume of the irradiated GTV:
      • 'Irradiated GTV' := any ROI whose name contains 'gtv' (case-insensitive)
        AND Min dose > 10 Gy. If multiple match, choose the largest volume (cc).
      • For healthy brain rows (name contains 'brain' and not 'brainstem'),
        Vx_cc := max( Vx_cc - GTV_volume_cc, 0 ).
    """
    rows = []
    hb_thrs = [5, 10, 12, 18, 20, 23, 24, 25, 27, 30]

    # ── PASS 0: find ALL irradiated targets (GTV/CTV with Min dose > 10 Gy) and sum their volumes ──
    total_treated_target_volume_cc = 0.0
    target_hits = []
    for roi_name, mask in masks.items():
        rn = roi_name.lower()
        # consider both GTV and CTV names (handles variants like "GTV1", "CTV Boost", etc.)
        if ("gtv" in rn) or ("ctv" in rn):
            dose_vals, w = _sample_native_dose(mask, spacing_mm, dose_arr, smooth_sigma)
            if dose_vals.size == 0:
                continue
            vol_cc = float(np.sum(w) * voxel_vol_cc)
            min_gy = float(dose_vals.min())
            if min_gy > 10.0:
                target_hits.append((roi_name, min_gy, vol_cc))
                total_treated_target_volume_cc += vol_cc

    if total_treated_target_volume_cc > 0:
        log.emit("🎯 Irradiated targets selected (Min > 10 Gy):")
        for n, m, v in target_hits:
            log.emit(f"   • {n}: Min={m:.2f} Gy, Vol={v:.2f} cc")
        log.emit(f"   → Total target volume subtracted from healthy brain Vx: {total_treated_target_volume_cc:.2f} cc")
    else:
        log.emit("ℹ️ No GTV/CTV met Min > 10 Gy; healthy brain Vx reported unadjusted.")

    # ── PASS 1: compute metrics (with adjusted Healthy Brain Vx) ─────────────
    for roi, mask in masks.items():
        dose_vals, w = _sample_native_dose(mask, spacing_mm, dose_arr, smooth_sigma)
        if dose_vals.size == 0:
            continue
        ...
        d2 = _weighted_percentile(dose_vals, w, 98.0)
        d50 = median
        d98 = _weighted_percentile(dose_vals, w, 2.0)
        ...
        roi_l = roi.lower()
        if whole_brain:
            # For WBRT: CI is defined on the Brain, not PTV
            roi_is_target = ("brain" in roi_l and "brainstem" not in roi_l)
        else:
            roi_is_target = ("ptv" in roi_l)

        if prescription and roi_is_target:
        ...
        if "brain" in roi_clean and "brainstem" not in roi_clean:
            for thr in hb_thrs:
                vx_raw = float(np.sum(w[dose_vals >= thr]) * voxel_vol_cc)
                vx_adj = max(vx_raw - total_treated_target_volume_cc, 0.0)
                row[f"V{thr}_cc"] = vx_adj

        else:
            # For non-brain ROIs keep their Vx (if ever needed in future) unmodified (optional)
            pass
...
def build_roi_masks(rs: pydicom.dataset.FileDataset,
    ...
    for roi in getattr(rs, "StructureSetROISequence", []):
        ...
        if should_skip_roi(name):
            if verbose:
                print(f"⏭️  Skipping ROI '{name}' (in skip list)")
            continue
...
class MetricsWorker(QThread):
    ...
    def run(self):
        try:
            ...
            plan_txt_path = os.path.join(self.patient_output_dir, "plan_metadata.txt")
            ...
            if os.path.exists(roi_path) and os.path.exists(meta_path) and dvh_files:
                self.log.emit("📂 Found existing extraction files. Loading instead of recomputing...")

                roi_df = pd.read_csv(roi_path)
                meta_df = pd.read_csv(meta_path)

                # Get prescription dose from the metadata table
                rx = None
                try:
                    rx_val = meta_df.loc[meta_df["Field"] == "Prescription Dose [Gy]", "Value"].iloc[0]
                    rx = float(rx_val)
                except Exception:
                    rx = None
            ...
            # Decide if this is a Whole Brain plan
            is_wb = is_whole_brain_treatment(self.patient_data, rp)

            if is_wb:
                self.log.emit(
                    "💊 Whole Brain plan detected – reading prescription from RP (TargetPrescriptionDose / DeliveryMaximumDose)...")
                rx_val = wb_prescription_from_rp(rp)
                if rx_val is not None:
                    rx = int(round(rx_val))
                    self.log.emit(f"   → RP prescription = {rx_val:.2f} Gy (stored as {rx} Gy)")
                else:
                    # Fallback if RP does not contain the expected fields
                    raw_max = float(np.max(dose_arr))
                    rx = int(round(raw_max * 0.8))
                    self.log.emit("   ⚠️ RP missing TargetPrescriptionDose / DeliveryMaximumDose;")
                    self.log.emit(
                        f"   → Falling back to round(max_dose × 0.8). Max dose = {raw_max:.2f} Gy, Rx = {rx} Gy")
            else:
                self.log.emit("💊 Computing prescription dose from max dose × 0.8...")
                raw_max = float(np.max(dose_arr))
                rx = int(round(raw_max * 0.8))
                self.log.emit(f"   → Max dose = {raw_max:.2f} Gy")
                self.log.emit(f"   → Prescription dose = round(max_dose × 0.8) = {rx} Gy")

            # max dose is taken from the resampled dose array (Gy)
            raw_max = float(np.max(dose_arr))

            # prescription dose = round(max_dose * 0.8)
            rx = int(round(raw_max * 0.8))
            ...
            roi_df = compute_roi_metrics(
                masks, dose_arr, vv_cc, rx, spacing, SMOOTH_SIGMA_MM, self.log,
                whole_brain=is_wb
            ).round(2)

            self.log.emit("📄 Building patient summary table (ID, Date, type, region, planning SW, fractions, Rx)...")
            meta_df = build_patient_summary_table(self.patient_data, rx, rp, whole_brain=is_wb)
            ...
            # Also save a human-readable plan metadata text file
            # (this corresponds to the left table in the GUI)
            lines = []
            lines.append("Plan metadata")
            lines.append("==============")
            lines.append("")

            max_field_len = int(meta_df["Field"].astype(str).str.len().max())
            for _, row in meta_df.iterrows():
                field = str(row["Field"])
                value = row["Value"]
                lines.append(f"{field:<{max_field_len}} : {value}")
            ...
            self.log.emit(f"💾 Saved plan metadata text to {plan_txt_path}")
...
def parse_patient_metadata(name: str):
    """
    Parse metadata from a patient/session folder name.

    Folder-based patterns are SECONDARY – date & treatment
    are taken from RP when possible.

    Examples handled:
      • 1575_SRS_LT occipital_02062021
      • 13JAN 21 SRS FRONTAL
      • FSR 18FEB2019
      • SRS LT PERIVENT 18FEB2019
      • 2334 wbrt
      • 26682
    """
    meta = {
    ...
    def build_date(day: str, mon_str: str, year: str) -> str:
        mon_num = mon_map.get(mon_str.upper(), "01")
        year = year.strip()
        if len(year) == 2:
            year = "20" + year
        day = day.zfill(2)
    ...
    # 1) 1575_SRS_LT occipital_02062021
    pattern1 = (
        r"(\d+)[ _]+([A-Z]+)"          # ID + Treatment
        r"(?:[ _]+([A-Z0-9]+))?"       # optional side
        r"(?:[ _]+([A-Za-z0-9 ]+))?"   # optional region
        r"[ _]+(\d{2})(\d{2})(\d{4})"  # ddmmyyyy
    )
    m1 = re.match(pattern1, s)
    if m1:
        pid, treatment, side, region, day, month, year = m1.groups()
        meta["Patient ID"] = pid
        meta["Treatment"]  = treatment.upper()
        meta["Region"]     = f"{side} {region}".strip() if side else (region or "Unknown")
        meta["Date"]       = f"{day}/{month}/{year}"
        return meta

    # 2) 13JAN 21 SRS FRONTAL
    pattern2 = r"^(\d{1,2})([A-Za-z]{3})\s*(\d{2,4})[ _-]+([A-Za-z]+)[ _-]*(.*)$"
    m2 = re.match(pattern2, s)
    if m2:
        day, mon_str, year, treatment, region = m2.groups()
        meta["Treatment"] = treatment.upper()
        meta["Region"]    = region.strip() or "Unknown"
        meta["Date"]      = build_date(day, mon_str, year)
        return meta

    # 3) FSR 18FEB2019   /   SRS LT PERIVENT 18FEB2019
    pattern3 = r"^([A-Za-z]+)\s*(.*)\s+(\d{1,2})([A-Za-z]{3})(\d{2,4})$"
    m3 = re.match(pattern3, s)
    if m3:
        treatment, region, day, mon_str, year = m3.groups()
        meta["Treatment"] = treatment.upper()
        meta["Region"]    = region.strip() or "Unknown"
        meta["Date"]      = build_date(day, mon_str, year)
        return meta

    # 4) ID only or "ID something":  "26682", "2334 wbrt"
    pattern4 = r"^(\d+)(?:\s+.*)?$"
    m4 = re.match(pattern4, s)
    if m4:
        meta["Patient ID"] = m4.group(1)
        # Leave treatment / date / region as Unknown – they will be
        # filled from RP (RTPlanDate, RTPlanLabel/Name) by the caller.
        return meta
...
def is_whole_brain_treatment(patient_folder_path: str,
                             rp: Optional[pydicom.dataset.FileDataset]) -> bool:
    """
    Heuristically decide if this plan is a Whole Brain plan.

    We look at the parsed folder metadata (Treatment / Region) and also at
    RTPLAN text fields such as RTPlanLabel / RTPlanName / RTPlanDescription.
    """
    folder_name = Path(patient_folder_path).name
    meta = parse_patient_metadata(folder_name)

    texts = []
    for k in ("Treatment", "Region"):
        v = meta.get(k)
        if v:
            texts.append(str(v))

    if rp is not None:
        for tag in ("RTPlanLabel", "RTPlanName", "RTPlanDescription", "PlanIntent"):
            if hasattr(rp, tag):
                texts.append(str(getattr(rp, tag)))

    s = " ".join(texts).lower()

    # More explicit phrases first
    if "whole brain" in s or "wbrt" in s:
        return True

    # Common shorthand such as "WB", "WB+C1-C2", etc.
    if s.startswith("wb") or " wb " in s or " wb+" in s or " wb-" in s or " wb/" in s:
        return True

    return False


def wb_prescription_from_rp(rp: pydicom.dataset.FileDataset) -> Optional[float]:
    """
    For Whole Brain plans, try to read the prescription from RP:
    prefer Target Prescription Dose, then Delivery Maximum Dose
    for DoseReferenceType == 'TARGET'.
    """
    try:
        for dr in getattr(rp, "DoseReferenceSequence", []):
            dr_type = str(getattr(dr, "DoseReferenceType", "")).upper()
            if dr_type != "TARGET":
                continue
            for tag in ("TargetPrescriptionDose", "DeliveryMaximumDose"):
                if hasattr(dr, tag):
...
def fractions_from_rp(rp: pydicom.dataset.FileDataset) -> Optional[int]:
    """
    Extract NumberOfFractionsPlanned from FractionGroupSequence.
    """
    try:
...
def extract_plan_date_from_rp(rp: Optional[pydicom.dataset.FileDataset]) -> Optional[str]:
    """
    Primary source for Session Date.
    Uses (300A,0007) RT Plan Date (format: YYYYMMDD).
    """
    if rp is None:
        return None
    try:
        raw = getattr(rp, "RTPlanDate", "") or rp.get((0x300A, 0x0007), "").value
    except Exception:
        raw = getattr(rp, "RTPlanDate", "")
    if not raw:
        return None
...
def extract_treatment_type_from_rp(rp: Optional[pydicom.dataset.FileDataset]) -> Optional[str]:
    """
    Primary source for Treatment Type.
    Looks into:
      (300A,0002) RT Plan Label
      (300A,0003) RT Plan Name

    Maps to: 'SRS', 'FSR', 'WB'
    (WB covers WBRT / whole brain; we search for 'wb', 'wbrt', 'whole brain').
    """
    if rp is None:
        return None

    label = str(getattr(rp, "RTPlanLabel", "") or "").lower()
    name  = str(getattr(rp, "RTPlanName", "") or "").lower()
    txt   = f"{label} {name}"

    if "srs" in txt:
        return "SRS"
    # also catch FSRT, etc.
    if "fsr" in txt or "fsrt" in txt:
        return "FSR"
    if "wbrt" in txt or "whole brain" in txt or re.search(r"\bwb\b", txt):
        return "WB"

    return None

def build_patient_summary_table(patient_folder_path: str,
                                prescription_gy: float | int,
                                rp: Optional[pydicom.dataset.FileDataset] = None,
                                whole_brain: bool = False) -> pd.DataFrame:
    folder_path = Path(patient_folder_path)
    folder_name = folder_path.name
    meta_from_name = parse_patient_metadata(folder_name)

    # ----- Patient ID from parent folder if missing -----
    pid = meta_from_name.get("Patient ID", "Unknown")
    if not pid or pid == "Unknown":
        parent_name = folder_path.parent.name
        m = re.match(r"(\d+)", parent_name)
        if m:
            pid = m.group(1)
            meta_from_name["Patient ID"] = pid

    # ----- Session Date & Treatment Type -----
    date_from_rp   = extract_plan_date_from_rp(rp) if rp is not None else None
    session_date   = date_from_rp or meta_from_name.get("Date", "Unknown")

    treat_from_rp  = extract_treatment_type_from_rp(rp) if rp is not None else None
    treatment_type = treat_from_rp or meta_from_name.get("Treatment", "Unknown")

    # ----- Planning Software -----
    planning_sw = "Unknown"
    if rp is not None:
        manu = getattr(rp, "Manufacturer", "").strip().lower()
        if "varian" in manu:
            planning_sw = "VMAT"
        elif "brainlab" in manu:
            planning_sw = "iPlan"

    rows = [
        ("Patient ID",        pid if pid else "UnknownID"),
        ("Session Date",      session_date if session_date else "UnknownDate"),
        ("Treatment Type",    treatment_type if treatment_type else "Unknown"),
        ("Planning Software", planning_sw),
    ]

    # ----- Number of Fractions -----
    # Show for WBRT and FSR plans (and keep old whole_brain flag for safety)
    if rp is not None and (whole_brain or treatment_type in ("WB", "FSR")):
        n_frac = fractions_from_rp(rp)
        rows.append(("Number of Fractions", n_frac if n_frac is not None else "Unknown"))

    # ----- Prescription dose -----
    rows.append(("Prescription Dose [Gy]", float(prescription_gy)))
...
class ArnousUnifiedGUI(QWidget):
    ...
    def load_patient_data(self):
        ...
        folder_path = Path(self.patient_data)
        self.patient_name = folder_path.name

        # --- Determine Patient ID from folder or parent ---
        meta = parse_patient_metadata(self.patient_name)
        pid = meta.get("Patient ID")
        if not pid or pid == "Unknown":
            parent_name = folder_path.parent.name
            m = re.match(r"(\d+)", parent_name)
            if m:
        ...
        # --- Build output directory name including Patient ID ---
        if pid and pid != "Unknown":
            if str(self.patient_name).startswith(str(pid)):
                folder_label = self.patient_name   # already has ID
            else:
                folder_label = f"{pid} {self.patient_name}"
        else:
            folder_label = self.patient_name

        self.patient_output_dir = os.path.join(
            self.outputs_dir,
            folder_label,
        ...
        # Show basic info (detailed meta is built during metrics extraction)
        info_text = f"   • Input Path: {self.patient_data}"
    ...
    def batch_process(self):
        ...
        root = Path(batch_data)
        patients_files = []

        # Accept BOTH layouts:
        # 1) Old strict layout: <BATCH>/<PATIENT>/CT/<series...>
        # 2) TABM-style layout: <BATCH>/<PATIENT>/<SESSION>/(dicoms here)

        for patient_dir in sorted([d for d in root.iterdir() if d.is_dir()]):

            # Try this folder itself (handles when DICOMs are directly under it)
            sessions = self._discover_session_folders(patient_dir)
            if sessions:
                patients_files.extend([str(s) for s in sessions])
                continue

            # Otherwise, look one level deeper (TABM: patient has session subfolders)
            for session_dir in sorted([d for d in patient_dir.iterdir() if d.is_dir()]):
                sessions = self._discover_session_folders(session_dir)
                if sessions:
                    patients_files.extend([str(s) for s in sessions])
