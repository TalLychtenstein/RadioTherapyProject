...
# ──────────────────────  Relative‑dose cumulative DVH  ───────────────────
def dvh_table_relative(dose_values, weights,
                       voxel_vol_cc: float,
                       prescription: float | None,
                       ...
                       max_dose: float | None = None) -> pd.DataFrame:
    """
    Return a 3‑column cumulative DVH table in *relative dose* space.

        Relative dose [%]   Dose [Gy]   Ratio of Total Structure Volume [%]

    • Relative dose = (Dose_Gy / prescription) *100   (NaN if no Rx).
    • Ratio volume  = (cum_volume / total_structure_volume) *100.
    """
    d_abs, v_abs = _dvh_cumsum_weighted(
        dose_values, weights, voxel_vol_cc, step, max_dose)

    if d_abs.size == 0:
        return pd.DataFrame(
            columns=["Relative dose [%]", "Dose [Gy]",
                     "Ratio of Total Structure Volume [%]"])

    rel_dose = (d_abs / prescription * 100.0
                if prescription else np.full_like(d_abs, np.nan, dtype=float))
    ratio_vol = v_abs / total_volume_cc * 100.0

    return pd.DataFrame({
        "Relative dose [%]": rel_dose,
        "Dose [Gy]": d_abs,
        "Ratio of Total Structure Volume [%]": ratio_vol
    })
# ────────────────────────────────────────────────────────────────────────
...
def compute_roi_metrics(masks: dict[str, np.ndarray],
    ...
    for roi, mask in masks.items():
        ...
        d2 = _weighted_percentile(dose_vals, w, 2)
        d50 = median
        d98 = _weighted_percentile(dose_vals, w, 98)
        mode = _weighted_mode(dose_vals, w)

        # Homogeneity Index
        hi = (d98-d2) / prescription if prescription else np.nan
        ...
        # Healthy‑brain Vx’s (only for the brain ROI)
        if roi.strip().lower() == healthy_brain_name.strip().lower():
            for thr in hb_thrs:
...
def main():
    if len(sys.argv) < 2:
        sys.exit("Usage:  python aligned_metrics_full.py  /path/to/patient_folder")
    ...
    print(f"Using fixed smooth_sigma = {SMOOTH_SIGMA_MM}mm")
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as xl:
        # ── 1) keep the ROI‑metrics sheet (if you still call compute_roi_metrics)
        metrics_df = compute_roi_metrics(
            masks, dose_arr, voxel_vol_cc,
            prescription, spacing_mm,
            smooth_sigma=SMOOTH_SIGMA_MM
        )
        metrics_df.to_excel(xl, sheet_name="ROI metrics", index=False)

        # ── 2) for every ROI: absolute‑dose sheet + relative‑dose sheet
        for roi, df_abs in dvh_abs.items():
            # Absolute‑dose cumulative DVH (already computed)
            sheet_name = "".join(c for c in roi if c not in r'\/:*?[]')[:31] or "ROI"
            df_abs.to_excel(xl, sheet_name=sheet_name, index=False)

            # Relative‑dose cumulative DVH
            dose_vals, w = _sample_native_dose(masks[roi], dose_arr)
            total_vol = np.sum(w) * voxel_vol_cc
            df_rel = dvh_table_relative(
                dose_vals, w, voxel_vol_cc,
                prescription=prescription,
                total_volume_cc=total_vol,
                step=0.1,
                max_dose=float(dose_arr.max())
            )
            # sheet name ≤31chars; add suffix “_rel”
            rel_name_raw = (roi[:28] + "_rel") if len(roi) > 28 else roi + "_rel"
            rel_name = "".join(c for c in rel_name_raw if c not in r'\/:*?[]')[:31] or "ROI_rel"
            df_rel.to_excel(xl, sheet_name=rel_name, index=False)

    print(f"✔️  Absolute + relative DVHs saved → {out_xlsx}")
