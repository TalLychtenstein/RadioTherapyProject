...
# Use the sigma-fixed engine everywhere (mm→voxel smoothing)
...
class ArnousViewer(QWidget):
    def __init__(self):
        ...
        # Make both tables interactive: drag to reorder columns/rows, resize columns/rows, sort, etc.
        def _make_table_interactive(tbl: QTableView):
            # Columns: allow drag-reorder + interactive resizing
            hh = tbl.horizontalHeader()
            hh.setSectionsMovable(True)
            hh.setSectionsClickable(True)
            hh.setSectionResizeMode(QHeaderView.Interactive)  # user-resizable
            hh.setStretchLastSection(False)  # keep true widths when moving

            # Rows: allow drag-reorder by grabbing the row header
            vh = tbl.verticalHeader()
            vh.setSectionsMovable(True)
            vh.setSectionsClickable(True)
            vh.setSectionResizeMode(QHeaderView.Interactive)  # user-resizable rows

            # General table UX: sorting & selection feel
            tbl.setSortingEnabled(True)  # click header to sort
            tbl.setSelectionBehavior(QAbstractItemView.SelectRows)
            tbl.setSelectionMode(QAbstractItemView.ExtendedSelection)
            tbl.setAlternatingRowColors(True)

        # Call it for both tables
        _make_table_interactive(self.tbl_meta)
        _make_table_interactive(self.tbl_roi)
